...
public class LeitoresController : ControllerBase
{
    ...
    public LeitoresController(SeBlogContext Context)
    {
        context = Context;
    }
    ...
    public async Task<ActionResult<IEnumerable<TbUsuario>>> Get()
    {
        try
        {   
            return Ok(await context.TbUsuarios.Where(p=> p.TipoUsuario == "L").ToListAsync());
        }
        catch
        {
            return BadRequest("Erro ao obter leitores");
        }
    }
